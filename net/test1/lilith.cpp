...
Network::Network() : layer1(648, 1024), layer2(1024, 1024),
                     policy_head(1024, 81), value_head(1024, 1) {}
...
float Network::train(const Matrix& input, const std::vector<float>& pi, float z, float lr) {
    ...
    Matrix d_h2_from_policy = policy_head.backward(d_policy_raw, lr);  // (1×1024)
    // value_head 反向，得到对 h2_post 的梯度
    Matrix d_h2_from_value = value_head.backward(d_value_raw, lr);     // (1×1024)
    ...
    // ---- 6. 第二隐藏层的线性反向 ----
    Matrix d_h1_post = layer2.backward(d_h2_pre, lr);  // (1×1024)
